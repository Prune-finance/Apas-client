...
const Company = forwardRef<HTMLDivElement, CompanyProps>(function Company(
  ...
  const handlePopulateForm = (data: BeneficiaryAccountProps) => {
    ...
    if (switchCurrency === "EUR") {
      ...
      if (data?.accountIban && data?.swiftBic) {
        ...
        form2.setFieldValue("routingNumber", data.routingNumber || "");
      ...
      if (isBank) {
        ...
        form2.setFieldValue("destinationBank", bankName);
        form2.setFieldValue("accountNumber", data.walletId || "");
        ...
        if (banks && Array.isArray(banks)) {
          ...
          if (selectedBank) {
            form2.setFieldValue("beneficiaryBankCode", selectedBank.bankCode);
          }
        }
      } else {
        setTransferCurrency("MobileMoney");
        const providerName = data?.mobileOperator || "";
        form2.setFieldValue("destinationBank", providerName);
        form2.setFieldValue("phoneNumber", data.walletId || "");
  ...
  useEffect(() => {
    form2.setFieldValue("usdTransferType", switchCurrencyOutsideUS);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const FullNameField = useCallback(
    ({
      form2,
      mt = 24,
    }: {
      form2: UseFormReturnType<typeof sendMoneyRequest>;
      mt?: number;
    }) => (
      <Flex gap={20} mt={mt}>
        <TextInput
          ...
          {beneficiaryAccount && beneficiaryAccount?.length > 0 ? (
            <Box mt={10} mb={32}>
              ...
              <Group mt={12} gap={24} wrap="nowrap">
                ...
                {beneficiaryAccount && beneficiaryAccount.length > 6 && (
                  <Stack
                    align="center"
                    gap={6}
